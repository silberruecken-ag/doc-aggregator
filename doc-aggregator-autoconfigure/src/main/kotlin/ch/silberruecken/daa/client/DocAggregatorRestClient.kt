...
class DocAggregatorRestClient(private val restClient: RestClient, private val applicationName: String) : DocAggregatorClient {
    override fun updateDocumentation(documentation: Documentation) {
        restClient.put().uri("/api/documentations") // TODO: Share uri and body in shared module
...
data class CreateDocumentationDto(val type: DocumentationType, val service: String, val uri: URI, val version: Version?) {
    companion object {
        fun fromDomain(documentation: Documentation, applicationName: String) = documentation.run {
            CreateDocumentationDto(DocumentationType.valueOf(type.toString()), applicationName, url, version)
        }
    }
}
